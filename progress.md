...
## 2026-07-31（移除 Capital/外形语音并精确校正四、五）
- 解决 iPad 英文声线将单个大写字符读成 `Capital A` 的问题：
  - 改为稳定的英文字母名发音串，例如 A=`ay`、B=`bee`、E=`ee`。
  - 大写和小写使用同一标准字母音，仅由前置中文“大写/小写”区分。
- 从字母专属贴纸包移除旧的“字母外形”贴纸，避免触发“外形”语音；已保存的非英文贴纸语音也会回退到字母默认单词。
- 使用 Hanzi Writer 逐笔中线数据重新校正：
  - 四：竖、横折、撇、竖弯、横。
  - 五：横、竖、横折、横；第二笔延伸到下部，第三笔横折独立完成。
- 验证：
  - 本地 SVG 渲染预览通过。
  - `npx tsc --noEmit` 通过。
  - `npm run build` 通过。
